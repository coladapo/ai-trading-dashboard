...
for i in range(0, len(tickers), 3):
    ...
    for j, ticker in enumerate(row):
        with cols[j]:
            ...
            support_df, resistance_df = detect_support_resistance(df)
            ...
            if view_option == "Chart View":
                ...
                fig.add_trace(go.Scatter(
                    x=support_df['Date'] if 'Date' in support_df else support_df['Datetime'],
                    y=support_df['Low'],
                    mode='markers',
                    name='Support',
                ...
                fig.add_trace(go.Scatter(
                    x=resistance_df['Date'] if 'Date' in resistance_df else resistance_df['Datetime'],
                    y=resistance_df['High'],
                    mode='markers',
                    name='Resistance',
                ...
                table_df = df[['Date', 'Open', 'High', 'Low', 'Close', 'sma']].tail(10) if 'Date' in df else df[['Datetime', 'Open', 'High', 'Low', 'Close', 'sma']].tail(10)
